...
Todas as ações usadas nos workflows/templates gerados por este pacote (`actions/checkout`, `actions/setup-node`, `actions/setup-java`, `actions/setup-dotnet`, `subosito/flutter-action`, `fkirc/skip-duplicate-actions`) estão fixadas por SHA de 40 carateres (`uses: actions/checkout@<sha> # v5.1.0`), não por tag móvel — é exatamente o que o `p/github-actions` verifica (`github-actions-mutable-action-tag`), e o próprio gate ficaria vermelho em si mesmo se não cumprisse a sua própria regra.

Há 3 findings aceites como falsos positivos/comportamento intencional, documentados no próprio código para quem for investigar uma falha do gate:

* **`bin/install.js`**: `detect-child-process` na função `run(cmd)` — `cmd` é sempre uma string fixa interna, nunca input externo; suprimido com `nosemgrep` e um comentário a explicar.
* **`mobile-release.yml`**: `workflow-run-target-code-checkout` no passo de checkout — o `if` do job já restringe a `head_branch == 'main'`, nunca faz checkout de uma PR externa; suprimido com `nosemgrep` e um comentário a explicar.
* **`templates/ci/dotnet-node-docker-e2e.yml`**: o valor por omissão de `POSTGRES_PASSWORD` (`'ci-e2e-password'`, só para CI) parece um segredo o suficiente para o gitleaks o assinalar como `generic-api-key`. Allowlisted no `.gitleaksignore` da raiz deste repositório, pelo fingerprint `commit:file:rule:linha` do commit que o introduziu. **Aviso para quem instala este template noutro projeto**: o gitleaks aí vai gerar um fingerprint diferente (commit próprio) para o mesmo falso positivo — o `.gitleaksignore` deste repositório não cobre esse caso; acrescenta a entrada equivalente ao `.gitleaksignore` do projeto de destino se o gate acusar isto.
...
Um projeto de plugin Claude Code ou Codex (mesma deteção usada pela [sincronização de plugins](#sincronização-de-plugins): `.claude-plugin/`, `.codex-plugin/` ou `.agents/plugins/marketplace.json` rastreados pelo Git) nunca recebe `ci.yml`, mesmo que tenha um script `test` num `package.json` ou corresponda a um template específico — o `package.json` desse tipo de projeto é tooling interno do plugin, não uma aplicação a testar via este workflow.
